...
const PLANS = [
  { id:'free',    name:'Free',    price:0,    period:null,    credits:'3 total',    features:['3 one-time credits','720p quality','Watermark on videos'] },
  { id:'starter', name:'Starter', price:999,  period:'month', credits:'20 / month', features:['20 credits/month','1080p quality','No watermark','Email support'] },
  { id:'pro',     name:'Pro',     price:2499, period:'month', credits:'60 / month', features:['60 credits/month','1080p quality','No watermark','Priority generation','Blog to video'], popular:true },
  { id:'creator', name:'Creator', price:4999, period:'month', credits:'150 / month',features:['150 credits/month','4K quality','No watermark','Fastest generation','API access (soon)'] },
]
...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    billingApi.history().then(d => setHistory(d.payments || [])).catch(() => {})
  }, [])
        ...
        <div className="p-7">
          <div className="mb-6">
            ...
            <p className="text-sm mt-0.5" style={{ color: 'var(--th-text-4)' }}>
              ...
              <span className="font-semibold" style={{ color: 'var(--th-accent)' }}>{user?.credits ?? '—'}</span> credits
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {PLANS.map(plan => {
              ...
              return (
                <div key={plan.id} className="card p-5 flex flex-col relative" style={plan.popular ? { borderColor: 'var(--th-accent)', boxShadow: '0 0 0 1px var(--th-accent)' } : {}}>
                  {plan.popular && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                      <span className="badge-purple text-xs font-bold px-3 py-1 rounded-full">Most popular</span>
                    </div>
                  )}
                  {isActive && <div className="absolute top-3 right-3"><span className="badge-green">Active</span></div>}
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-3xl font-black" style={{ color: 'var(--th-text-1)' }}>{plan.price.toLocaleString()}</span>
                      {plan.period && <span className="text-xs" style={{ color: 'var(--th-text-4)' }}>/{plan.period}</span>}
                    </div>
                    <div className="text-sm font-semibold mt-0.5" style={{ color: 'var(--th-accent)' }}>{plan.credits}</div>
                  </div>
                  ...
                  >
                    {isActive ? 'Current plan' : plan.id === 'free' ? 'Free plan' : paying === plan.id ? 'Processing…' : `Upgrade to ${plan.name}`}
                  </button>
                </div>
              )
            })}
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--th-border)' }}>
                      {['Plan','Amount','Credits','Date','Status'].map(h => (
                        <th key={h} className="text-left text-xs font-semibold uppercase tracking-wide px-5 py-3" style={{ color: 'var(--th-text-4)' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {history.map(p => (
                      <tr key={p.id} style={{ borderBottom: '1px solid var(--th-border)' }}>
                        ...
                        <td className="px-5 py-3" style={{ color: 'var(--th-text-2)' }}>KES {(p.amount / 100).toLocaleString()}</td>
                        <td className="px-5 py-3 font-semibold" style={{ color: 'var(--th-accent)' }}>{p.credits_granted}</td>
                        <td className="px-5 py-3" style={{ color: 'var(--th-text-4)' }}>{new Date(p.created_at).toLocaleDateString()}</td>
                        <td className="px-5 py-3"><span className={p.status === 'active' ? 'badge-green' : 'badge-gray'}>{p.status}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
